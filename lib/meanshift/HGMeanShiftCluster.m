...
stopThresh = 1e-4*bandWidth; % when mean has converged
...
while numInitPts
    ...
    while true %loop untill convergence
        if numDim == 1
            sqDistToAll = bsxfun(@minus,myMean,dataPts).^2; % dist squared from mean to all points still active
        else
        ...
        if plotFlag
            ...
            if numDim == 1
                plot(ones(length(dataPts), 1), dataPts, '.')
                plot(myMean, '*', 'LineWidth', 2.0)
                plot(myOldMean, 'o', 'LineWidth', 2.0)
